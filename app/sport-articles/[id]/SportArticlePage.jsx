// app/sport-articles/[id]/SportArticlePage.jsx
...
const SportArticlePage = ({ params }) => {
  ...
  // Fetch and process the sport article data as per your logic
  ...
  const sportArticleData = {
    title: "Sample Sport Article Title",
    description: "Sample Sport Article Description",
    date: "2023-10-01T00:00:00Z",
    content: '<p>This is a <a href="https://example.com">link</a> in the main text.</p>',
    // ... other fields
  };

  // Process sport article data
  sportArticleData.formattedDate = moment(sportArticleData.date).format('LL');
  sportArticleData.title = decode(sportArticleData.title);
  const sanitizedContent = getSanitizedContent(sportArticleData.content);
        ...
        <div className="w-full h-auto mb-5">
          ...
          {showLanguageDropdown && (
            <DynamicClientComponents
              id={id}
              title={sportArticleData.title}
              showLanguageDropdown={showLanguageDropdown}
            />
          )}
        </div>

        <h1 className="text-2xl font-bold">{sportArticleData.title}</h1>
        <p className="text-gray-600">{sportArticleData.formattedDate}</p>
        <div
          className="prose"
