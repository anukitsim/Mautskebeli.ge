...
const Language1Page = async ({ params }) => {
  const { id } = params;
  const article = await fetchArticle(id);
  ...
  article.title.rendered = decodeHTMLEntities(article.acf.language1_title);

  // Sanitize and process the language1 main text
  let sanitizedContent = sanitizeHtml(article.acf.language1_main_text, {
