...
const BookPage = ({ params }) => {
  ...
  useEffect(() => {
    if (article && articleContentRef.current) {
      ...
      articleContent.innerHTML = DOMPurify.sanitize(article.acf['text']);
  ...
  const articleUrl = `https://www.mautskebeli.ge/books/${article.id}`;
  ...
  const ogDescription = sanitizeDescription(article.acf['text']).slice(0, 150);

  const currentCategory = "მაუწყებელი წიგნები"; 
