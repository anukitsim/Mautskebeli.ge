...
const BookPage = ({ params }) => {
  ...
  const generatePermalink = (bookId) => {
    return `https://mautskebeli.wpenginepowered.com/mau-books/${bookId}`;
  };

  const shareOnFacebook = () => {
    const permalink = generatePermalink(book.id);
  ...
  const shareOnInstagram = () => {
    alert(
      "Instagram sharing is not supported directly via web. Please use the Instagram app."
    );
  };
      ...
      <div className="w-full lg:w-[54%] mx-auto bg-opacity-90 p-5 rounded-lg">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: DOMPurify.sanitize(book.acf["text"]),
          }}
        ></div>
        {book.acf["notes"] && (
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: DOMPurify.sanitize(book.acf["notes"]),
            }}
          ></div>
        )}
