    ...
    const fetchArticles = async (page = 1, articles = []) => {
      try {
        const response = await fetch(
          `https://mautskebeli.wpenginepowered.com/wp-json/wp/v2/mau-books?acf_format=standard&_fields=id,title,acf,date&per_page=20&page=${page}`
        );
        if (!response.ok) {
