    ...
    const fetchArticles = async (page = 1, articles = []) => {
      try {
        const response = await fetch(
          `https://mautskebeli.wpenginepowered.com/wp-json/wp/v2/free-column?acf_format=standard&_fields=id,title,acf,date&per_page=20&page=${page}`
        );
        if (!response.ok) {
    ...
    <section className="mx-auto flex flex-col overflow-hidden">
      <div className="w-full sm:w-11/12 md:w-10/12 lg:w-10/12 xl:w-9/12 mx-auto">
        ...
        <p className="text-[#8D91AB] text-[14px] font-bold pl-2 pt-5 w-9/12 text-justify mb-10">
          ...
          „მაუწებელი“ მიზნად ისახავს მხარი დაუჭიროს ადგილობრივი ხმებისა და
          თვალსაზრისების გაჟღერებას და თავისუფალ პლატფორმას უთმობს ავტორებს,
