...
function XelovnebaVideos() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/wp/v2/msoflio?per_page=100`
        );
        if (!response.ok) {
        ...
        {loading ? (
          ...
          videos.length > 0 && (
            <>
              ...
              >
                                 <h1 className="text-[#474F7A] text-[24px] font-bold mt-5 lg:mt-20 pl-4 mb-10 lg:ml-36 lg:pl-2 ">მსოფლიო</h1>
